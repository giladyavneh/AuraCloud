...
- **Comments explain _why_, never _what_ — and almost never.** The default is no comment: a clear name beats a sentence explaining an unclear one. Comment only a non-obvious constraint, a bug being prevented, or a deliberate trade-off, and keep it to **one line**. If it needs a paragraph or a multi-line JSDoc block, the code is too unclear — fix the code instead. Never narrate what the code does, never leave progress notes, and never cite documents the reader does not have (spec sections, tickets, chat history).
