...
import { AwsResourceModel, ResourceActionModel, UserModel, encryptSecret } from "utils";
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    const existing = await CustomerModel.findOne({ email: (email as string).toLowerCase().trim() }).lean();
    if (existing) {
    ...
    if (role === "manager") {
      ...
      if (!companyName || !companySlug) {
        res.status(400).json({ message: "companyName and companySlug are required for managers" });
        return;
      }
      ...
      if (!companySlug || !inviteCode) {
        res.status(400).json({ message: "companySlug and inviteCode are required for employees" });
        return;
      }
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    const customer = await CustomerModel.findOne({ email: (email as string).toLowerCase().trim() });
    if (!customer) {
      res.status(401).json({ message: "Invalid email or password" });
      return;
    }

    const valid = await bcrypt.compare(password as string, customer.passwordHash);
    if (!valid) {
      res.status(401).json({ message: "Invalid email or password" });
      return;
    }
...
app.post("/api/aws/onboard-credentials", requireAuth, async (req, res) => {
  try {
    ...
    if (customer.role !== "manager") {
      res.status(403).json({ message: "Only managers can update AWS credentials" });
      return;
    }
...
app.get("/api/company/invite-code", requireAuth, async (req, res) => {
  try {
    ...
    if (customer.role !== "manager") {
      res.status(403).json({ message: "Only managers can view the invite code" });
      return;
    }
    const company = await CompanyModel.findById(customer.companyId, { inviteCode: 1, slug: 1 }).lean();
    if (!company) {
      res.status(404).json({ message: "Company not found" });
      return;
    }
